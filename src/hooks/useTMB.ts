...
// TMB ("Trusted Member Backend" silent-SSO) relies on first-party cookies
// shared across *.deriv.com/.deriv.me/.deriv.be (plus local QA domains).
// Its `sessions/active` endpoint is intentionally CORS-blocked for any other
// origin — that is expected, not a bug on Deriv's side. Previously this hook
// still attempted the fetch on third-party domains (e.g.
// <your-vercel-domain>), which produced console CORS errors and, worse,
// could leave the login button stuck in the TMB code path instead of falling
// through to the working OIDC/PKCE login (`requestOidcAuthentication`).
// Short-circuiting here removes the network call entirely on those domains.
const isFirstPartyDerivDomain = (): boolean => {
    if (typeof window === 'undefined') return false;
    const first_party_domains = ['deriv.com', 'deriv.dev', 'binary.sx', 'pages.dev', 'deriv.be', 'deriv.me'];
    const hostname = window.location.hostname;
    if (hostname === 'localhost') return true;
    const current_domain = hostname.split('.').slice(-2).join('.');
    return first_party_domains.includes(current_domain);
};
...
const useTMB = (): UseTMBReturn => {
    ...
    const getActiveSessions = useCallback(async (): Promise<TMBWebsocketTokens | undefined> => {
        if (!isFirstPartyDerivDomain()) {
            // Never call oauth.deriv.com/oauth2/sessions/active from a
            // third-party origin — it will always be blocked by CORS there.
            // Treat it the same as "no active SSO session" without making
            // the network request at all.
            return undefined;
        }
        try {
            ...
            // v5.5.4 — downgrade to soft warning. This endpoint is redirected
            // by Deriv on non-*.deriv.com origins (e.g. <your-vercel-domain>),
            // which browsers block during CORS preflight. That is EXPECTED for
            // third-party domains; treat it as "no active SSO session" and
            // continue booting instead of spamming console.error.
            if (typeof console !== 'undefined' && console.debug) {
                console.debug('[TMB] no active SSO session (expected on non-deriv.com origins):', (error as any)?.message ?? error);
            }
            return undefined;
        }
    }, []);
    ...
    const isTmbEnabled = useCallback(async () => {
        ...
        tmbStatusPromiseRef.current = (async () => {
            try {
                // TMB is only meaningful on first-party Deriv domains (it
                // depends on cookies shared with oauth.deriv.com). On any
                // other domain, force it off so the login flow always uses
                // the working OIDC/PKCE path instead of the CORS-blocked
                // sessions/active check.
                if (!isFirstPartyDerivDomain()) {
