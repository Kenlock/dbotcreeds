...
export default class ActiveSymbols {
    ...
    async retrieveActiveSymbols(is_forced_update = false) {
        ...
        // v5.5.4 — wrap in try/catch so a bad payload never bricks the app
        try {
            this.processed_symbols = this.processActiveSymbols();
        } catch (e) {
            if (typeof console !== 'undefined' && console.debug) {
                console.debug('[active-symbols] categoriser threw, falling back to empty map:', e && e.message);
            }
            this.processed_symbols = {};
        }
    ...
    processActiveSymbols() {
        // v5.5.4 hardening — the Deriv legacy WebSocket occasionally returns
        // active_symbols entries that are missing required fields (this happens
        // on the very first response before the socket fully authorises, or
        // when an alphanumeric app_id was mistakenly used on the numeric-only
        // legacy WS). Skip any malformed entry instead of crashing the whole
        // categoriser with "Cannot read properties of undefined (reading
        // 'submarket_display_name')".
        const list = Array.isArray(this.active_symbols) ? this.active_symbols : [];
        return list.reduce((processed_symbols, symbol) => {
            if (!symbol || typeof symbol !== 'object') return processed_symbols;
            if (!symbol.symbol || !symbol.market || !symbol.submarket) return processed_symbols;
            if (
            ...
            if (!isExistingValue(symbols, 'symbol')) {
                symbols[symbol.symbol] = {
                    ...
                    pip_size: symbol.pip != null ? `${symbol.pip}`.length - 2 : 4,
